...
//! [`ForAudience`]. Validators implement [`Validate`] (from
//! [no-way-jose-core](https://docs.rs/no-way-jose-core)) and compose with
//! [`Validate::and_then`]. Signed and encrypted tokens are represented in core
//! by [`CompactJws`](https://docs.rs/no-way-jose-core/latest/no_way_jose_core/type.CompactJws.html)
//! and [`CompactJwe`](https://docs.rs/no-way-jose-core/latest/no_way_jose_core/type.CompactJwe.html).
//!
//! This is the only crate in the workspace that depends on `std`; it uses
//! [jiff](https://docs.rs/jiff) for timestamps (`exp`, `nbf`, `iat`), which
//! serialize as integer seconds (`NumericDate`) on the wire.
//!
//! # Getting started
//!
//! [`RegisteredClaims::new`] seeds `iat` and `nbf` from `now` and sets `exp` to
//! `now + ttl`. Use the builder methods to fill issuer, subject, audience, and
//! optional `jti`:
//!
//! ```
...
//! # Custom validation
//!
//! Chain rules with [`Validate::and_then`]. For claim structs that embed
//! [`RegisteredClaims`], reuse the same validators via [`Validate::map`], which
//! selects the nested field before running the inner rule:
//!
//! ```
//! # fn main() -> Result<(), jiff::Error> {
//! use jiff::SignedDuration;
...
//! ```
//!
//! A typical flat pipeline on [`RegisteredClaims`] alone looks like this:
//!
//! ```
//! # fn main() -> Result<(), jiff::Error> {
//! use jiff::SignedDuration;
